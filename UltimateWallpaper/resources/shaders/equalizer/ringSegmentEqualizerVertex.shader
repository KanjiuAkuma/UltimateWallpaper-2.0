...
void main() {
	float angle = dTheta + theta * (gl_InstanceID + in_position.x);
	float amplitude = texelFetch(u_spectrum, gl_InstanceID).x;
	float amp = u_baseAmplitude + texelFetch(u_spectrum, gl_InstanceID).x;

	vec2 pos = rotate(vec2(0.f, u_innerRadius + amp * in_position.y * outerRadius), angle);
